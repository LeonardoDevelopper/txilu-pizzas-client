...
const ViewWaitingShipping = () => {
    ...
    const get_order_waiting_payment = async () => {
        ...
        .then(res => res.json())
        .then((response) => {
            setList(response.data.filter((item) => item.STATUS == 'Paid'))
        })
        .catch((error) => {
            alert(error.message) 
        })
    ...
    return (
        <View style={{backgroundColor: 'white'}}>
            <HeaderScreen title={"Aguardando envio"} target1={'Status'} />
            <View style={{
                top: 90,
...
export default ViewWaitingShipping